...
export default function handler(
  ...
  const generateAlias = ({ minLength = 6, maxLength = 20, separator = '-', numWords = 3 }) => {
    // Split lyrics into individual words, normalize them and filter out empty words
    const words = lyrics.split(/\s+/).map(normalizeWord).filter(Boolean);
  ...
  const generatedAlias = generateAlias({ minLength: 6, maxLength: 20, separator: '.', numWords: 3 })
  console.log({generatedAlias})
  
  res.status(200).json({ alias: generatedAlias });
}
